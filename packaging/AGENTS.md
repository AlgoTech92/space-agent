...
`packaging/desktop/updater_debug_release.js` is the canonical packaged-desktop helper for test-only updater staging. It resolves release tags and platform metadata filenames, fetches updater metadata from the canonical GitHub Release assets, and stages same-version reinstalls or specific-version downgrades back through the normal `electron-updater` download and install flow instead of inventing a second installer path.
