...
- `_core/spaces` currently uses the sections seam to inject a space-local `## Current Space Agent Instructions` section when the current space defines agent instructions, plus a live `## Current Open Space` block while the routed spaces canvas is open, sourcing widget ids, names, order, positions, sizes, and state from `space.current` and advertising metadata-first `space.current.readWidget(...)` plus renderer-line `space.current.patchWidget(...)` for compact widget-definition inspection and patching
...
- the firmware prompt should also remind the agent to `return await ...` for browser mutations that need confirmation, should bias existing-widget work toward `readWidget(...)` plus `patchWidget(...)` instead of direct file reads or full `renderWidget(...)` rewrites, should explain that `readWidget(...)` returns plain metadata plus zero-based numbered renderer lines and that `patchWidget(...)` edits only those renderer lines while metadata changes use explicit inputs, should explicitly use `return await space.current.renderWidget(...)` only for new widgets or deliberate full rewrites, should prefer renderer signatures like `async (parent, currentSpace) => { ... }` so the global `space` runtime stays unshadowed, should keep widget-rendering guidance short and example-driven, should point widget authors at `space.utils.markdown.render(text, target)` for simple markdown output inside widgets, should refer to the active thread as `space.chat`, should use `space.utils.yaml.parse(...)` plus `space.utils.yaml.stringify(...)`, and should keep the current spaces widget-size ceiling (`24x24`) explicit
...
- `execution-retry`: the overlay is generating an empty-assistant protocol-correction retry message
...
- in compact mode, the floating UI bubble should stream the assistant reply live on each delta without waiting for completion, but it must render only the text before the execution separator
...
- `execution.js` owns the execution transcript contract: successful no-return runs must emit `no result returned, no console logs`, result blocks must print `result:` on its own line before the raw returned text, and string or object results must not be clipped into `...` before the model sees them
